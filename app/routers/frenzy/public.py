...
async def get_active_event(
    ...
    template = (await session.execute(select(FrenzyTemplate).where(FrenzyTemplate.id == event.template_id))).scalar_one_or_none()
    if template is None:
        raise HTTPException(500, "Event template not found.")

    teams = (await session.execute(select(FrenzyTeam).where(FrenzyTeam.event_id == event.id).order_by(FrenzyTeam.sort_order))).scalars().all()

    pending_by_team: dict[int, list[FrenzySubmission]] = {}
    pending_rows = (await session.execute(
        select(FrenzySubmission).where(FrenzySubmission.event_id == event.id, FrenzySubmission.status == "pending")
    )).scalars().all()
    ...
    for team in teams:
        ...
        if pending:
            pi, pa, pm = _apply_pending_submissions(team.item_progress or {}, team.activity_progress or {}, team.milestone_progress or {}, pending)
            pending_points = _compute_scores_from_progress(template, pi, pa, pm)["total"] - scores["total"]
        else:
            pending_points = 0.0

        teams_out.append({
            "id": team.id, "name": team.name, "slug": team.slug, "icon_url": team.icon_url,
            "sort_order": team.sort_order, "total_points": scores["total"],
            "pending_points": pending_points, "tier_points": scores["tier_points"],
            "activity_points": scores["activity_points"], "milestone_points": scores["milestone_points"],
        })

    teams_out.sort(key=lambda t: t["total_points"], reverse=True)
    return {
        "id": event.id, "name": event.name, "wom_comp_id": event.wom_comp_id,
        "starts_at": event.starts_at.isoformat() if event.starts_at else None,
        "ends_at": event.ends_at.isoformat() if event.ends_at else None,
        "template_id": event.template_id, "teams": teams_out,
    }
...
async def get_active_team_detail(
    ...
    pending_subs = (await session.execute(
        select(FrenzySubmission).where(FrenzySubmission.team_id == team.id, FrenzySubmission.status == "pending")
    )).scalars().all()

    if pending_subs:
        pi, pa, pm = _apply_pending_submissions(team.item_progress or {}, team.activity_progress or {}, team.milestone_progress or {}, pending_subs)
        pending_points = _compute_scores_from_progress(template, pi, pa, pm)["total"] - scores["total"]
    else:
        pending_points = 0.0

    approved_subs = (await session.execute(
        select(FrenzySubmission).where(FrenzySubmission.team_id == team.id, FrenzySubmission.status == "approved")
    )).scalars().all()
            ...
            if source.get("name") == src_name:
                for item in source.get("items", []):
                    if item.get("name") == item_name:
                        ...
                        break
        player_contribution[sub.player_rsn] = player_contribution.get(sub.player_rsn, 0.0) + pts

    return {
        "id": team.id, "name": team.name, "slug": team.slug, "icon_url": team.icon_url,
        "participants": team.participants or [],
        "template": {
            "tiers": template.tiers, "activities": template.activities,
            "milestones": template.milestones, "multipliers": template.multipliers,
        },
        "progress": {
        ...
        "scores": scores, "pending_points": pending_points,
