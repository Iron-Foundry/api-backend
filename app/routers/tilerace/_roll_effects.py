...
def find_cell(cells: list, position: int) -> dict | None:
    for cell in cells:
...
def cell_is_gated(cell: dict | None) -> bool:
    return bool(cell and cell.get("tile_id"))


def apply_landing_modifiers(team: TileRaceTeam, cell: dict | None) -> dict:
    summary: dict = {}
...
def cell_in_pad(cell: dict | None, pad: dict | None) -> bool:
...
def apply_pad_trigger(team: TileRaceTeam, pad: dict | None, summary: dict) -> None:
...
def _apply_one(
    modifier: dict, team: TileRaceTeam, effects: dict, summary: dict
) -> None:
