...
from ._constants import (
    _COMP_METRIC_STALE_TTL, _COMPS_FRESH_KEY, _COMPS_FRESH_TTL, _COMPS_LOCK_KEY,
    _COMPS_STALE_KEY, _COMPS_STALE_TTL, _WOM_API_KEY, _WOM_DISCORD_CONTACT, _WOM_GROUP_ID,
)
...
async def _build_competitions_cache(valkey: Valkey) -> None:
    ...
    try:
        ...
        if comps:
            ...
            logger.info("competitions cache: wrote {} competitions ({})", len(comps), ", ".join(f"{v} {k}" for k, v in statuses.items()))
        else:
...
async def _build_metric_detail_cache(comp_id: int, metric: str, status: str, valkey: Valkey) -> None:
    ...
    try:
        ...
        payload = json.dumps({
            "id": data["id"], "title": data["title"], "metric": metric,
            "type": data.get("type", "classic"), "status": status,
        ...
        logger.info("comp metric cache: wrote comp={} metric={} participants={}", comp_id, metric, len(raw_parts))
    except Exception as exc:
        logger.error("comp metric cache: hydration failed comp={} metric={} - {}", comp_id, metric, exc)
    finally:
