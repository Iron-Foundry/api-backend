...
from ._constants import (
    _COMP_METRIC_LOCK_TTL, _COMP_METRIC_MAP_KEY, _COMPS_FRESH_KEY, _COMPS_LOCK_KEY,
    _COMPS_LOCK_TTL, _COMPS_STALE_KEY, _GLOBAL_GUILD_ID, _WOM_API_KEY, _WOM_DISCORD_CONTACT,
)
...
async def get_competition_metric_map(session: AsyncSession = Depends(get_session)) -> dict:
...
@router.post("/competitions/metric-map", dependencies=[Depends(require_page_permission("staff.competitions", "edit"))])
async def set_competition_metric_map(
    ...
    await session.execute(
        ...
        .on_conflict_do_update(index_elements=["guild_id", "key"], set_={"value": current})
    )
...
async def list_competition_participants(session: AsyncSession = Depends(get_session)) -> list[dict]:
    """Return all users with linked RSNs for competition participant autofill."""
    result = await session.execute(select(User.rsn, User.discord_username).where(User.rsn.is_not(None)))
    return [{"rsn": row.rsn, "discord_username": row.discord_username} for row in result]
...
async def competition_metric_detail(
    ...
    if await valkey.set(lock_key, "1", ex=_COMP_METRIC_LOCK_TTL, nx=True):
        background_tasks.add_task(_build_metric_detail_cache, competition_id, metric, status, valkey)
...
async def competition_overtime(
    ...
    db_result = await session.execute(
        ...
        .where(CompetitionSnapshot.comp_id == competition_id, CompetitionSnapshot.metric == metric)
        ...
        if status == "finished" or age < timedelta(minutes=35):
            ...
            for snap in db_snaps:
                for standing in snap.series:
                    ...
                    players.setdefault(name, []).append({"date": snap.captured_at.isoformat(), "value": standing["gained"]})
            series = sorted(
...
async def competition_details(
    ...
    participations = [
        {"player_name": p["player"]["displayName"], "team_name": p.get("teamName"), "progress": p.get("progress", {}), "levels": p.get("levels", {})}
        for p in data.get("participations", [])
    ]
    return {
        "id": data["id"], "title": data["title"], "metric": metric, "type": data.get("type"),
        "startsAt": data["startsAt"], "endsAt": data["endsAt"], "groupId": data.get("groupId"),
        "participantCount": len(participations), "score": data.get("score"), "status": status,
