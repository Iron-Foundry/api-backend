...
async def login() -> RedirectResponse:
    state = jwt.encode(
        {"nonce": uuid4().hex, "exp": datetime.now(timezone.utc) + timedelta(minutes=5)},
        JWT_SECRET,
...
async def callback(
    ...
    async with httpx.AsyncClient() as client:
        ...
        me = me_resp.json()
        guilds_resp = await client.get(f"{_DISCORD_API}/users/@me/guilds", headers=headers)
    ...
    await session.execute(
        pg_insert(User)
        .values(discord_user_id=discord_user_id, discord_username=me.get("username", ""), discord_roles=discord_roles, roles_fetched_at=now, guild_id=int(GUILD_ID) if GUILD_ID else 0, created_at=now, updated_at=now)
        .on_conflict_do_update(index_elements=["discord_user_id"], set_={"discord_username": me.get("username", ""), "discord_roles": discord_roles, "roles_fetched_at": now, "updated_at": now})
    )

    ticket_ids = await get_user_ticket_ids(session, discord_user_id)
    if ticket_ids:
        await session.execute(update(User).where(User.discord_user_id == discord_user_id).values(ticket_ids=ticket_ids))
        logger.debug("auth/callback: synced {} ticket(s) for user {}", len(ticket_ids), discord_user_id)

    await session.commit()
    token = issue_jwt(discord_user_id=str(discord_user_id), username=me.get("username", ""), avatar=me.get("avatar"))
