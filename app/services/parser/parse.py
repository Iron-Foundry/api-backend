...
def classify(message: str) -> BroadcastType:
    ...
    if P.COMBAT_ACH.match(message) or P.COMBAT_TIER.match(message) or P.COMBAT_TIER_UNLOCK.match(message):
...
def parse_loot(message: str) -> ParsedLoot | None:
    ...
    if m := P.RAID_LOOT.match(message):
        return ParsedLoot(player_name=m.group("player"), item_name=m.group("item"), coin_value=None, source="raid")
    return None
...
def parse_level_up(message: str) -> ParsedLevelUp | None:
    ...
    if m := P.LEVEL_IN.match(message):
        ...
        return ParsedLevelUp(player_name=m.group("player"), skill=m.group("skill"), new_level=int(level_str) if level_str else 99)
    if m := P.LEVEL_OF.match(message):
        return ParsedLevelUp(player_name=m.group("player"), skill=m.group("skill"), new_level=int(m.group("level")))
    return None
...
def parse_xp_milestone(message: str) -> ParsedXpMilestone | None:
    ...
    if m := P.XP_MILESTONE.match(message):
        return ParsedXpMilestone(player_name=m.group("player"), skill=m.group("skill"), xp=int(m.group("xp").replace(",", "")))
    return None
...
def parse_achievement(message: str) -> ParsedAchievement | None:
    ...
    if m := P.COMBAT_ACH.match(message):
        ...
        return ParsedAchievement(player_name=m.group("player"), kind="combat_achievement", name=m.group("name"), difficulty=diff.lower() if diff else None)
    if m := P.COMBAT_TIER.match(message):
        diff = m.group("difficulty").lower()
        return ParsedAchievement(player_name=m.group("player"), kind="combat_achievement", name=f"{diff} tier", difficulty=diff)
...
def parse_loot_key(message: str) -> ParsedLootKey | None:
    ...
    if m := P.LOOT_KEY.match(message):
        return ParsedLootKey(player_name=m.group("player"), coin_value=int(m.group("value").replace(",", "")))
    return None
...
def parse_clue_item(message: str) -> ParsedClueItem | None:
    ...
    if m := P.CLUE_ITEM.match(message):
        raw_value = m.group("value")
        return ParsedClueItem(player_name=m.group("player"), item_name=m.group("item"), coin_value=int(raw_value.replace(",", "")) if raw_value else None)
    return None
...
def parse_pk(message: str) -> ParsedPk | None:
    ...
    if m := P.PK_WINNER.match(message):
        return ParsedPk(winner=m.group("winner"), loser=m.group("loser"), gp_exchanged=int(m.group("gp").replace(",", "")))
    if m := P.PK_LOSER.match(message):
        raw_gp = m.group("gp")
        return ParsedPk(winner=m.group("winner"), loser=m.group("loser"), gp_exchanged=int(raw_gp.replace(",", "")) if raw_gp else None)
    return None
...
def parse_league_area(message: str) -> ParsedLeagueArea | None:
    ...
    if m := P.LEAGUE_AREA.match(message):
        nth = m.group("nth")
        return ParsedLeagueArea(player_name=m.group("player"), area_count=int(nth) if nth else None)
    return None
