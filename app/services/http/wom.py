...
class WiseOldManHandler(WomCompetitionMixin, WomGroupMixin, WomPlayerMixin, BaseRequestHandler):
    ...
    def __init__(
        ...
        *,
        ...
        user_agent = f"IronFoundry/1.0 (discord: @{discord_contact})" if discord_contact else "IronFoundry/1.0"
    ...
    async def _get_with_rate_limit(self, path: str, *, params: dict | None = None) -> httpx.Response:
        """GET via priority queue with proactive sleep and 429 retry."""
        queue = get_wom_queue()
        resp: httpx.Response | None = None
        for attempt in range(3):
            resp = await queue.submit(lambda p=path, pa=params: self.get(p, params=pa), self._priority)
    ...
    async def _write_with_rate_limit(self, method: str, path: str, *, json: dict | None = None) -> httpx.Response:
        ...
        for attempt in range(3):
            ...
            if resp.status_code != 429:
                return resp
            retry_after = float(resp.headers.get("retry-after", "5"))
            logger.warning("wom: 429 on {} {} (attempt {}) - sleeping {:.1f}s", method.upper(), path, attempt + 1, retry_after)
            await asyncio.sleep(retry_after)
        return resp  # type: ignore[return-value]
