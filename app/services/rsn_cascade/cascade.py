...
async def _cascade_rsn_single(
    ...
    await session.execute(
        update(User).where(func.lower(User.rsn) == old_rsn.lower()).values(rsn=new_rsn, updated_at=now)
    )
    ...
    await session.execute(
        update(Leaderboard).where(func.lower(Leaderboard.player_name) == old_rsn.lower()).values(player_name=new_rsn)
    )
    ...
    await session.execute(delete(PlayerRanking).where(func.lower(PlayerRanking.rsn) == old_rsn.lower()))
    ...
    await session.execute(delete(PlayerSnapshot).where(func.lower(PlayerSnapshot.rsn) == old_rsn.lower()))

    await session.execute(
        update(FrenzySubmission).where(func.lower(FrenzySubmission.player_rsn) == old_rsn.lower()).values(player_rsn=new_rsn)
    )
    await session.execute(
        update(PartyDB).where(func.lower(PartyDB.leader_rsn) == old_rsn.lower()).values(leader_rsn=new_rsn)
    )
    await session.execute(
        update(PartyMemberDB).where(func.lower(PartyMemberDB.rsn) == old_rsn.lower()).values(rsn=new_rsn)
    )
    await session.execute(
        update(PartyChatMessageDB).where(func.lower(PartyChatMessageDB.rsn) == old_rsn.lower()).values(rsn=new_rsn)
    )
...
async def cascade_rsn_change(session: AsyncSession, old_rsn: str, new_rsn: str) -> None:
    ...
    user_result = await session.execute(
        select(UserAccount.discord_user_id).where(func.lower(UserAccount.rsn) == old_rsn.lower())
    )
    ...
    await session.execute(
        ...
        .values(rsn=new_rsn, rsn_history=func.array_append(UserAccount.rsn_history, old_rsn))
    )
